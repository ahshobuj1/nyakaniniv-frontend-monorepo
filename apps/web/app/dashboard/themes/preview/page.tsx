  ...
  return (
    <div className="flex flex-col h-screen bg-[#f1f5f9]">
      ...
      <header className="bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between shrink-0 shadow-sm z-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-medium">Viewing theme with default content</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button 
            onClick={() => router.push(`/dashboard/manage-theme?themeId=${themeId}`)}
      ...
      <main className="flex-1 overflow-y-auto bg-slate-100 p-4 md:p-8 lg:p-12">
        <div className="max-w-[1400px] mx-auto bg-white rounded-2xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] overflow-hidden border border-white">
          <TemplateRenderer
            templateId={themeId}
...
export default function ThemePreviewPage() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center min-h-screen bg-slate-50 text-slate-400">Loading preview...</div>}>
      <ThemePreviewContent />
    </Suspense>
  );
}
