    ...
    <div className="bg-[#f2f2f2] min-h-screen py-16 px-4 font-sans">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-center mb-6">
          <div className="flex items-center gap-1.5 bg-red-50 text-[#ff3b3b] px-3 py-1.5 rounded text-xs font-semibold border border-red-100">
            <Zap size={14} fill="currentColor" className="text-[#ff3b3b]" />
          ...
          <div className="bg-white p-1.5 rounded shadow-sm inline-flex items-center">
            <button
              ...
              className={`px-6 py-2 cursor-pointer rounded-[2px] text-sm font-semibold transition-all duration-300 ${
                frequency === 'monthly' ? 'bg-[#ff3b3b] text-white' : 'text-gray-900'
              }`}
            >
              Monthly
            </button>
            <button
              ...
              className={`px-6 py-2 cursor-pointer rounded-[2px] text-sm font-semibold flex items-center gap-2 transition-all duration-300 ${
                frequency === 'yearly' ? 'bg-[#ff3b3b] text-white' : 'text-gray-900'
              }`}
            >
              Annual
        ...
        <div className="grid md:grid-cols-3 gap-6 items-start">
          {plans.map((plan) => (
            <div key={plan.name} className={`relative ${plan.highlighted ? '' : 'bg-white p-8 shadow-sm'}`}>
              {plan.highlighted && (
              ...
              <div className={plan.highlighted ? 'bg-white p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] relative' : ''}>
                <h3 className="text-[28px] font-bold text-gray-900 mb-1">{plan.name}</h3>
                ...
                <button
                  className={`w-full py-2.5 font-bold mb-8 transition-colors duration-300 ${
                    plan.highlighted
                      ? 'bg-[#ff3b3b] text-white hover:bg-[#e63535]'
                ...
                <ul className="space-y-3.5">
                  {plan.features.map((feature, index) => (
                    <li key={index} className="flex items-center gap-3">
                      ...
                      <span className={feature.included ? "text-gray-600 text-sm" : "text-gray-400 text-sm"}>
                        {feature.name}
                      </span>
                    </li>
                  ))}
        ...
        <div className="mt-16 text-center text-gray-900 font-medium">
          Running a DJ agency or label? <a href="#" className="text-[#ff3b3b] hover:underline transition-colors duration-300">Contact us for Enterprise pricing &rarr;</a>
        </div>
      </div>
    </div>
  );
